...
Registration requires activation via a 5-digit code (logged to console in dev, in place of an email service) before login is allowed.

---

## User Flows

- **Register:** Submits email/password/name → account created inactive, default `BANK` account created alongside it, activation code logged to console → redirected to `/register/confirm` → enters the code at `/register/activate` (or requests a new one) → account activated → can now log in.
...
- **Forgot password:** Submits email at `/forgot-password` → backend silently no-ops if the email doesn't exist (avoids leaking which emails are registered) → reset link logged to console → `/reset-password?token=&email=` verifies the token, lets the user set a new password.
...
- Activation/reset "email" delivery is stubbed: both log the code/link to the console instead of sending real email. See `authService.ts`'s `logActivation` and `forgotPassword`.
...
- The `logActivation` and `forgotPassword` "email" delivery is dev-only console logging — a real deployment needs an actual email provider wired into `authService.ts` before this ships to real users.
