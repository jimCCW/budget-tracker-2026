...
describe('registerSchema', () => {
  ...
  it('rejects password without a number', () => {
    expect(
      registerSchema.safeParse({ ...valid, password: 'NoNumber!' }).success
    ).toBe(false);
  });

  it('rejects password without a special character', () => {
    expect(
      registerSchema.safeParse({ ...valid, password: 'NoSpecial1' }).success
    ).toBe(false);
  });
...
describe('loginSchema', () => {
  it('accepts valid credentials', () => {
    expect(
      loginSchema.safeParse({ email: 'u@ex.com', password: 'pw' }).success
    ).toBe(true);
  });
...
describe('activateSchema', () => {
  it('accepts valid 5-digit code', () => {
    expect(
      activateSchema.safeParse({ email: 'u@ex.com', code: '12345' }).success
    ).toBe(true);
  });

  it('rejects code with wrong length', () => {
    expect(
      activateSchema.safeParse({ email: 'u@ex.com', code: '123' }).success
    ).toBe(false);
  });
});
...
describe('forgotPasswordSchema', () => {
  it('accepts valid email', () => {
    expect(forgotPasswordSchema.safeParse({ email: 'u@ex.com' }).success).toBe(
      true
    );
  });
...
describe('resetPasswordSchema', () => {
  ...
  it('rejects weak password', () => {
    expect(
      resetPasswordSchema.safeParse({ ...valid, password: 'weak' }).success
    ).toBe(false);
  });
});
