...
describe('createRuleSchema', () => {
  ...
  it('accepts INCOME kind', () => {
    expect(
      createRuleSchema.safeParse({ ...validCreate, kind: 'INCOME' }).success
    ).toBe(true);
  });
  ...
  it('accepts all frequencies', () => {
    ...
    for (const frequency of freqs) {
      expect(
        createRuleSchema.safeParse({ ...validCreate, frequency }).success
      ).toBe(true);
    }
  });
  ...
  it('rejects invalid frequency', () => {
    expect(
      createRuleSchema.safeParse({ ...validCreate, frequency: 'HOURLY' })
        .success
    ).toBe(false);
  });
});
...
describe('updateRuleSchema', () => {
  ...
  it('accepts partial update', () => {
    expect(
      updateRuleSchema.safeParse({ amount: 200, frequency: 'WEEKLY' }).success
    ).toBe(true);
  });
});
