import {
  createExpenseSchema,
  updateExpenseSchema,
} from '../../../src/schemas/expenseSchemas';
...
describe('createExpenseSchema', () => {
  ...
  it('accepts optional description', () => {
    expect(
      createExpenseSchema.safeParse({ ...validCreate, description: 'Lunch' })
        .success
    ).toBe(true);
  });
  ...
  it('rejects missing categoryId', () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.error.errors[0].message).toBe('Category is required');
  });
  ...
  it('rejects negative amount', () => {
    expect(
      createExpenseSchema.safeParse({ ...validCreate, amount: -5 }).success
    ).toBe(false);
  });
  ...
  it('rejects invalid date string', () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.error.errors[0].message).toMatch(/valid ISO date/i);
  });
